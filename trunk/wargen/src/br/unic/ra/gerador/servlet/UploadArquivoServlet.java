	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ((contentType != null) && (contentType.indexOf("multipart/form-data") >= 0)) {
				...
				String caminho = request.getSession().getServletContext().getRealPath("\\ra");
				
				if(extensao.equals("dae")) {
			...
			request.getSession().setAttribute("erro", exc);
			exc.printStackTrace();
		}
		finally {
			if (fileOut != null) {
			...
			response.sendRedirect("/ra/usuario");
		}
	}
}
